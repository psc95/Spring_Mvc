  ...
  <c:if test="${!empty blist}">
   <c:forEach var="b" items="${blist}">
    <tr>
     ...
     <th>
     <a href="/controller/board/board_cont?bno=${b.title}&page=${page}">${b.title}</a>
     <%-- board_cont?bno=번호&page=쪽번호가 get방식으로 bno에 번호,page에 쪽번호가 &구분기호로 구분해서
     2개의 피라미터값이 get방식으로 전달된다. 이런 방식을 쿼리스트링 방식 이라고 한다.특히 쪽번호를 get으로 전달
     하는 이유는 책갈피 기능 구현 때문이다.ㄴ --%>
     </th>
     <th>${b.writer}</th>
     <th>${b.viewcnt}</th>
     <th>${b.regdate}</th>
