...
--각 게시판 번호에 해당하는 실제 댓글 수를 카운터 해서 새롭게 추가된 replycnt컬럼 레코드값으로 수정
...
,bno number(38) default 0 --게시판 번호,외래키로 추가 설정되어 져서 tbl_board테이블의 기본키로 설정된 bno컬럼 레코드 값인
--게시판 번호값만 이 컬럼 레코드 번호값으로 저장됨. 주종 관계이다.
...
nocycle; --시퀀스 최대값 번호 생성시 시퀀스 번호값 생성 중지(다시 1부터 반복안함)
