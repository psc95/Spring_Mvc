...
public class MessageController {
	...
	//메시지 추가
	@RequestMapping(value="/message_insert", method=RequestMethod.POST) //POST로 접근하는
	//매핑주소 처리, message_insert 매핑주소 등록
	public ResponseEntity<String> messageInsert(@RequestBody MessageVO vo) {
		/* @RequestBody MessageVO vo라고 하면 전송된 JSON 키,값 쌍의 데이터는 MessageVO 객체타입으로
		 * 변경된다.
		 */
		...
		try {
			...
			//예외 에러가 발생하면 에러 메시지와 나쁜 상태코드가 반환
		}
		return entity;
	}//messageInsert()
}
