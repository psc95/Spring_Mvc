...
@RestController //@스프링 4.0이후 부터는 @RestController 애노테이션을 사용함으로써 jsp와 같은 뷰페이지를
//만들지 않고도 Rest방식의 데이터를 처리할 수 있게 되었다.만들어진 데이터는 키,값쌍의 json, 문자열, xml등이다.
public class Sample2Controller {
	
	@RequestMapping("/rest_start") //get or post로 접근하는 매핑주소 처리, rest_start 매핑주소
	//등록
	...
	public ResponseEntity<Void> sendError(){
		...
		/* @RestController 는 별도의 JSP파일을 만들지 않고도 REST API 서비스를 실행하기 때문에
		 * 결과 데이터에 대한 예외 오류가 많이 발생한다.이런 경우는 스프링에서 제공하는 내장 API인
		 * ResponseEntity 타입을 사용해서 개발자에게 문제가 되는 HTTP 에러 상태 코드인404(파일없음),
		 * 500(오타에러)등과 같은 나쁜 상태 코드를 데이터와 함께 브라우저로 전송되어 져서 좀더 세밀한 에러 제어
		 * 를 할수 있게 한다.
		 * 200상태코드는 정상 상태코드이고 BAD_REQUEST는 400 나쁜 상태코드를 브라우저로 전송한다.
		 */
	}//sendError()
	
	//컬렉션 List 타입의 정상적인 json데이터와 404(파일없음) 나쁜상태코드를 동시에 브라우저로 전송
